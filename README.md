...
Powered by the **Google GenAI SDK** and the **Gemini 2.5 Flash** model, our `SmartScan` module evaluates images of UCO containers in real-time. It analyzes the visual characteristics to return a structured JSON assessment containing:

* **Quality Grade:** A (Premium), B (Standard), or C (Low-grade/Industrial).
* **Composition Analysis:** Evaluates purity percentage, water content levels, and detects physical debris.
* **Oil Type Identification:** Classifies the source (Vegetable, Animal Fat, Mixed).
...
A dedicated `B2BPortal` allowing restaurants and F&B operators to seamlessly book bulk oil pickups (>20 Liters), track their total recycled volume, and calculate their specific CO₂ offsets.
...
### 🇲🇾 5. Full Bilingual Localization (English & Bahasa Melayu)
To ensure accessibility for all Malaysians, the entire patform features dynamic language switching. Managed via a custom React context (useLanguage), the app instantly translates UI elements, AI reasoning outputs, and educational content between English and Bahasa Melayu (BM).

### 📖 6. Eco-Education Hub
A dedicated interactive module designed to change user habits. It educates users on the environmental hazards of improper UCO disposal ("The Silent Pollutant"), the circular economy of biodiesel, and provides step-by-step guides on proper oil filtration and storage. 
...
* **Cloud Backend & Auth:** Google Firebase v12 (Authentication, Real-time services)
* **AI Integration:** Google GenAI SDK (`@google/genai`)
* **Mapping:** Leaflet & React-Leaflet with marker clustering
* **State & i18n:** Custom React Context for seamless localized translations (`lib/translations.ts`)
